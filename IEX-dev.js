...
var StyleSheet = Base.extend({
  ...
  parse: function() {
    ...
    while ((rule = RULE.exec(this.cssText))) {
      var cssText = rule[2];
      if (cssText) {
        var fixDescendants = (MSIE5 || appVersion < 7) && cssText.indexOf("AlphaImageLoader") !== -1;
...
var Rule = IE7.Rule = Base.extend({
  ...
  parse: function(selector) {
    ...
    if (MSIE5 || appVersion < 7) simple = simple.replace(Rule.MULTI, "");
...
if(MSIE5 || appVersion < 7){
  IE7.PNG = {
    ...
    createVmlStyleSheet: function () { /* style VML, enable behaviors */
      ...
      */
      var styleSheet, printStyleSheet;
    ...
    **/
    fix: function(selector) {
      if (this.styleSheet) {
        ...
        for (i=0; i<selectors.length; i++) {
          this.styleSheet.addRule(selectors[i], 'behavior:expression(IE7.PNG.fixPng(this))'); /* seems to execute the function without adding it to the stylesheet - interesting... */
        }
      }
    },
    ...
    fixPng: function(el) {
      ...
      if ((thisStyle['width'] != 'auto' || thisStyle['height'] != 'auto') && (thisStyle['vnx-resize'] != 'no-resize') && el.isImg == true){el.vml.image.fill.type = 'frame';} else {el.vml.image.fill.type = 'tile';}
  ...
  IE7.PNG.createVmlNameSpace();
  IE7.PNG.createVmlStyleSheet();
  IE7.PNG.fix('*');
}
...
var HTML5 = "article,aside,audio,canvas,details,figcaption,figure,footer,header,hgroup,mark,main,menu,nav,output,section,summary,time,video".split(",");
...
article,aside,figcaption,footer,header,hgroup,main,summary,section,nav{display:block}\
                      ...
                      ? ((T = T.nodeValue),
                        T &&
                            ...
                            (T !== U.getAttribute(B) || ("password" === U.type && !U.getAttribute(D))) &&
                                ("password" === U.type && !U.getAttribute(D) && g(U, "text") && U.setAttribute(D, "password"), U.value === U.getAttribute(B) && (U.value = T), U.setAttribute(B, T))))
